...
func ServeHTTP(port int) {
	...
	r.GET("/configure", func(c *gin.Context) {
		...
		for i, field := range m.Config {
			switch field.Key {
			case "username":
				if config.Username != "" { m.Config[i].Default = config.Username }
			case "password":
			...
			case "sortingPreference":
				if config.SortingPreference != "" { m.Config[i].Default = config.SortingPreference }
			case "showQualities":
				if config.ShowQualities != "" { m.Config[i].Default = config.ShowQualities }
			case "maxResultsPerQuality":
				if config.MaxResultsPerQuality != "" { m.Config[i].Default = config.MaxResultsPerQuality }
			case "maxFileSize":
				if config.MaxFileSize != "" { m.Config[i].Default = config.MaxFileSize }
			case "uiLanguage":
				if lang != "" {
